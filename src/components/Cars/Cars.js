...
const Cars = () => {
    const {cars, prev, next, total_pages} = useSelector(state => state.cars);
    const dispatch = useDispatch();
    const [query, setQuery] = useSearchParams({page: '1'});

    useEffect(() => {
        dispatch(carActions.getAll({page: query.get('page')}))
    }, [dispatch, query, total_pages])

    return (

        <div>
            <div>
                {/*<button disabled={!prev} onClick={() => setQuery(prev)}>prev*/}
                <button disabled={!prev} onClick={() => setQuery(query => ({page: +query.get('page') - 1}))}>prev
                </button>
                {/*<button disabled={!next} onClick={() => setQuery(next)}>next*/}
